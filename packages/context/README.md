...
## :globe_with_meridians: Download Server API

Context supports downloading pre-built documentation packages from remote servers. The default server is `https://context.neuledge.com`, but anyone can host a compatible server.
...
| Tool | Description |
|------|-------------|
| `get_docs` | Search installed documentation (local, instant) |
| `search_packages` | Find available packages on the server |
| `download_package` | Download and install a package from the server |

### Server API Specification

A compatible download server must implement these HTTP endpoints:

#### `GET /search?registry=<r>&name=<n>&version=<v>`

Search for available packages. Returns a JSON array:
...
- `registry` (required): Package manager (e.g., `npm`, `pip`)
- `name` (required): Package name
- `version` (optional): Specific version filter

#### `GET /packages/<registry>/<name>/<version>`

Check if a package version exists. Returns `200` if it exists, `404` if not.

#### `GET /packages/<registry>/<name>/<version>/download`

Download the `.db` file. Returns the SQLite database as `application/octet-stream`.

#### `POST /packages/<registry>/<name>/<version>`

Upload a built `.db` file. Requires `Authorization: Bearer <key>` header. Body is the raw `.db` file with `Content-Type: application/octet-stream`.

### Server Configuration

Server configuration is stored in `~/.context/config.json`:
